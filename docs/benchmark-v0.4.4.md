...
| Max % | Min % | Shader | Notes |
|---|---|---|---|
| ~18–22 | ~14 | Temple | Retro temple interior — centered horizon; floor + ceiling share triangle-wave field (ceiling phase-offset by 3.7); 20 scrolling pillars (5 rows × 4 columns, outer ±4.0, inner ±1.5), structured columns (base + shaft + capital), single front face per pillar, `PILLAR_DRIFT_SCALE=0` removes drift multiply on pillar pixels, `is_pillar` bool skips horizon haze. Round 6: side face removed entirely; each pillar split into 3 vertical zones — base (bottom 15%, wider rect, horizontal bars + notch), shaft (middle, vertical lines), capital (top 15%, wider rect, horizontal bars; bracket flare at top 10%); widening sells 3D through silhouette variation rather than projected second face. Expected util: **~18–22%**, lower than round 5 (~20%) because side face loop iteration is gone and zone logic adds only a few simple conditionals per pillar. |

...
- **Waves estimate basis (post-tweak, hash decorrelation + luminance-preserving ceiling):** Single divide + min (perspective), 3 triangle-wave calls (each: 1 mul, 1 fract, 1 abs, 1 sub, 1 mul, 1 sub — ~6 ALU, no transcendentals), 1 fract + 1 abs + 1 step (isolines), 1 floor + 1 mul + 1 fract (band index / golden-ratio hash palette coord), 1 texture read (palette), 1 mul + 1 fract + 1 step (liveness hash — offline/online, constant change), 3× `max` (floor) + 2× `max` + 1 `min` + 1 mul (luminance-preserving ceiling — ~4 ALU vs ~6 for per-channel clamp, net slight reduction), 1 `exp()` + 1 mul (distance fog), 1 smoothstep (haze), 1 fract + 1 step (scanlines). `OFFLINE_HASH` constant change is zero GPU cost. Luminance-preserving scale is ~4 ALU vs 6 for prior per-channel clamp — marginal reduction. Expected delta from 16–18% baseline: ±1%. Estimate unchanged: 16–18% max.

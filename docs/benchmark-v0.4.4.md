...
| Max % | Min % | Shader | Notes |
|---|---|---|---|
| ~22–30 | ~18 | Temple | Retro temple interior — centered horizon; floor + ceiling share triangle-wave field (ceiling phase-offset by 3.7); 4 scrolling pillars (screen-space rect test per pillar, 4-iteration loop with early reject). Ceiling branch: ~+1%; pillar loop: ~+5–10%; pillar pixels: ~+2–3% above waves baseline. Estimate pending HawkPoint1 verification. |

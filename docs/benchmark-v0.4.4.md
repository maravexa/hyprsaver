...
| Max % | Min % | Shader | Notes |
|---|---|---|---|
| ~22–30 | ~18 | Temple | Retro temple interior — centered horizon; floor + ceiling share triangle-wave field (ceiling phase-offset by 3.7); 4 scrolling pillars with fixed corridor layout (outer ±3.5, inner ±1.0, paired z-phase staggering), vertical trace pattern (`pillar_u * LINE_DENSITY` only, no t/pillar_v terms), `is_pillar` bool skips horizon haze. Pattern change is ~0 ops vs. prior formula; layout change is identical pillar-check count. Expected util: ~17%, unchanged from pre-fix baseline. |
...
- **Temple estimate basis:** Inherits all waves ALU (perspective, 3 tri-waves, isolines, palette hash, liveness, fog, haze, scanlines). Additions: ceiling branch (~+1% — single bool + float add); 4-iteration pillar loop with early `continue` (~+5–10% — 4 × screen-rect test + conditional pillar UV); pillar-pixel rendering is same color pipeline as surface (~+2–3% for pixels inside pillar rects). Total delta from 16–18% waves baseline: +6–13%. Estimate: 22–30% max. Pillar fixes (layout + pattern + `is_pillar` haze): computationally identical — fixed x arithmetic replaces fract hash (same ALU count); `pillar_u * DENSITY` is one mul vs. three-term sum (negligible); `is_pillar ? 1.0 : smoothstep(...)` is a boolean branch/select (~0 ops on RDNA wavefront). Expected util unchanged at ~17%.

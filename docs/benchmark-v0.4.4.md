...
| Max % | Min % | Shader | Notes |
|---|---|---|---|
| ~20–24 | ~16 | Temple | Retro temple interior — centered horizon; floor + ceiling share triangle-wave field (ceiling phase-offset by 3.7); 12 scrolling pillars (3 rows × 4 columns, outer ±4.0, inner ±1.5), vertical trace pattern (`pillar_u * LINE_DENSITY`), cap bars at top/bottom 10% of each pillar (`step`+`mix`, trivial cost), `PILLAR_DRIFT_SCALE=0` removes drift multiply on pillar pixels, `is_pillar` bool skips horizon haze. Round 3: `PILLAR_LINE_DENSITY` 1.0→0.5 (3 lines per pillar), `PILLAR_ISOLINE_WIDTH=0.12` (pillar-specific wider isolines), `SCROLL_SPEED` 0.4→0.8. Delta from 17% baseline: +3–5% from 3× pillar count (partially offset by early-reject), +0–1% from cap zone mix, −0.5% from removed drift multiply. Round 3 neutral: fewer isoline evaluations (fewer lines), one ternary select for `iso_width`. Expected util: **20–24%**, unchanged. |

...
| Max % | Min % | Shader | Notes |
|---|---|---|---|
| ~30–33 | ~20 | Temple | Retro temple interior — centered horizon; floor + ceiling share triangle-wave field (ceiling phase-offset by 3.7); 20 scrolling pillars (5 rows × 4 columns, outer ±4.0, inner ±1.5). Round 10: `OFFLINE_RATIO` raised 0.4 → 0.7 (online traces ~30% of bands); base pattern simplified to plain horizontal bars (removed `tri(face_u * 4.0) * BASE_NOTCH_AMPLITUDE`). Expected util: **~30%**, unchanged or trivially lower (one `tri()` call removed per base-zone pixel). |
...
- **Temple estimate basis (round 10 — online ratio + base pattern match):** Baseline ~30–33% from round 9. `OFFLINE_RATIO` 0.4 → 0.7 is a constant change — zero GPU cost. Base pattern simplification removes one `tri()` call + one multiply per base-zone pixel, trivial cost reduction (<0.5%). Net: **~30%**, unchanged.

...
| Max % | Min % | Shader | Notes |
|---|---|---|---|
| ~28–34 | ~20 | Temple | Retro temple interior — centered horizon; floor + ceiling share triangle-wave field (ceiling phase-offset by 3.7); 20 scrolling pillars (5 rows × 4 columns, outer ±4.0, inner ±1.5). Round 7: hexagonal columns with 3-visible-face geometry — `hex_visible_faces()` (1 atan) selects center + 2 neighbor faces; each face projects its 2 corners via perspective divide, interpolates depth via inverse-z, applies base/shaft/capital zone pattern; depth-sorted closest-wins across all 3 faces and all 20 pillars. `HEX_FACE_COLOR_SHIFT=0.089` distinguishes adjacent faces. Expected util: **~28–34%**, up from ~20% baseline. Primary cost driver: atan + 4 sin + 4 cos per pillar (~180 trig ops/pixel at 20 pillars). Still comfortable Medium tier. |

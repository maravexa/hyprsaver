...
- **Cosine gradient palettes** — 12 floats define smooth, infinite color ramps. Any shader × any palette
...
- Configurable FPS, fade in/out, and dismiss triggers
...
> **Note**: hypridle respects `org.freedesktop.ScreenSaver.Inhibit` (set by most video players and browsers during full-screen playback), so hyprsaver is automatically suppressed while you watch a film. For apps that don't set the inhibitor, add a hotkey:
>
> ```ini
> # hyprland.conf
> bind = $mod, F7, exec, systemd-inhibit --what=idle --mode=block sleep infinity &
> ```
...
bind = $mod, F6, exec, hyprsaver --preview mandelbrot   # preview
bind = $mod, F7, exec, hyprsaver --quit                  # dismiss
```
...
A full annotated example is provided at [`config.example.toml`](config.example.toml).
...
[shaders]
shader_dir = "~/.config/hyprsaver/shaders"
hot_reload = true
fallback_on_error = true

[palettes.custom.my_palette]
a = [0.5, 0.5, 0.5]
b = [0.5, 0.5, 0.5]
c = [1.0, 1.0, 1.0]
d = [0.00, 0.33, 0.67]
...
where `a`, `b`, `c`, `d` are RGB vectors and `t ∈ [0, 1]`.
...
```
┌─────────────────────────────────────────────────────────────────┐
│  main.rs — CLI, signal handling, event loop (calloop)           │
└────────────────────────┬────────────────────────────────────────┘
                         │
         ┌───────────────┴───────────────┐
         │                               │
